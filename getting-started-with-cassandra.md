# Write your first application using Scalar DB with Cassandra
...
2 schema files are available for cassandra, depending if you want to run the application using the [Storage](#use-scalar-db-without-transaction) or [Transaction](#use-scalar-db-with-transaction) mode.

Storage :

```bash
java -jar $PATH_TO_SCALARDB/target/scalar-schema.jar --cassandra -h <CASSANDRA_IP> -u <CASSNDRA_USER> -p <CASSANDRA_PASSWORD> -f schema_storage_cassandra.json [-n <NETWORK_STRATEGY> -R <REPLICATION_FACTOR>]
```

Transaction :

```bash
java -jar $PATH_TO_SCALARDB/target/scalar-schema.jar --cassandra -h <CASSANDRA_IP> -u <CASSNDRA_USER> -p <CASSANDRA_PASSWORD> -f schema_transaction_cassandra.json [-n <NETWORK_STRATEGY> -R <REPLICATION_FACTOR>]
```
...
We define these tables in [schema_storage_cassandra.json](database/schema_storage_cassandra.json) in format of [Scalar DB database schema](https://github.com/scalar-labs/scalardb/blob/master/docs/schema.md).
