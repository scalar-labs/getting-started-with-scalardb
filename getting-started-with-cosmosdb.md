# Write your first application using Scalar DB with Cosmos DB
- [Write your first application using Scalar DB](#write-your-first-application-using-scalar-db)
    - [Set up the environment](#set-up-the-environment)
        ...
        - [Deploy the REST API](#deploy-the-rest-api)
            - [Configure the cosmosdb connection](#configure-the-cosmosdb-connection)
 ...
 In this tutorial we will use a sample Q&A application to demonstrate the functionalities of Scalar DB with Cosmos DB. The Q&A application is a web application frontend (single page application), backend (HTTP API), and a Scalar DB supported database.
    
![Q&A architecture overview_Cosmosdb](img/architecture_overview_cosmosdb.png)
...
Please install [Scalar DB prerequisites](https://github.com/scalar-labs/scalardb/blob/master/docs/getting-started-with-cosmosdb.md#install-prerequisites), [Node.js](https://nodejs.org/en/download/) and [yarn](https://yarnpkg.com/en/docs/install) before running this demonstration.

### Install the database model
    
To install the database schema, we will use the [schema tools](https://github.com/scalar-labs/scalardb/tree/master/tools/scalar-schema) of ScalarDB.
The schema files are located inside the [/database](Q%26A_application/database) folder.
...
```bash
java -jar $PATH_TO_SCALARDB/target/scalar-schema.jar --cosmos -h <YOUR_ACCOUNT_URI> -p <YOUR_ACCOUNT_PASSWORD> -f schema_storage_cosmosdb.json
```
    
Transaction :
    
```bash
java -jar $PATH_TO_SCALARDB/target/scalar-schema.jar --cosmos -h <YOUR_ACCOUNT_URI> -p <YOUR_ACCOUNT_PASSWORD> -f schema_transaction_cosmosdb.json
```
...
The backend will expose a REST API on http://localhost:8090. The address and port can be configured in the [application.properties](backend/QA/src/main/resources/application.properties).
...
To run the front end, run in the [**/frontend**](Q%26A_application/frontend) folder.
...
After having submitted a question, you will be redirected to the page with the question details where you can consult and add the answer.
...
- FirstQuestionDate : it records the day the first question was ever created. This field is used as the lower boundary when iterating through all of the questions recorded in the database. (Refer to the method `get(String start, int minimal)` in [QuestionServiceForStorage.java](Q%26A_application/backend/QA/src/main/java/com/example/qa/service/question/QuestionServiceForStorage.java) for more details)

We define these tables in [schema_storage_cosmosdb.json](database/schema_storage_cosmosdb.json) in format of [Scalar DB database schema](https://github.com/scalar-labs/scalardb/blob/master/docs/schema.md).
...
**Note** : refers to the class constructor and the *createDistributedStorage()* method of [LocallyConfiguredStorageFactory.java](Q%26A_application/backend/QA/src/main/java/com/example/qa/dao/LocallyConfiguredStorageFactory.java) for more details
...
**Note** : refers to the method *put(QuestionRecord record, DistributedTransaction transaction)* of [QuestionDao.java](Q%26A_application/backend/QA/src/main/java/com/example/qa/dao/question/QuestionDao.java) for more details.
...
**Note** : refers to the method *update(
      String date, long createdAt, long updatedAt, int numAnswer, DistributedStorage storage)* of [QuestionDao.java](Q%26A_application/backend/QA/src/main/java/com/example/qa/dao/question/QuestionDao.java)
      for more details.
...
**Note** : refers to the method *get(String date, long createdAt, DistributedStorage storage)* of [QuestionDao.java](Q%26A_application/backend/QA/src/main/java/com/example/qa/dao/question/QuestionDao.java)
      for more details.
...
**Note** : refers to the method *createDistributedTransactionManager()* of [LocallyConfiguredStorageFactory.java](Q&A_application/backend/QA/src/main/java/com/example/qa/dao/LocallyConfiguredStorageFactory.java) for more details.
